...
title = ['idx','category','title','date','summary','original']
writer.writerow(title) #- 컬럼명 추가

# 2. selenium 과 BeautifulSoup을 활용해 크롤링을 진행합니다.
browser = webdriver.Chrome('/Users/eunhasu/PycharmProjects/pythonProject/chromedriver') # selenium driver 크롬
...
for idx, card in enumerate(cards):
    ...
    for paragraph in paragraphs:
        try:
            ...
            # if 'orange_link' not in paragraph.find("a").attrs:
            #     url = paragraph.find("a", attrs={"target": "_blank"})["href"]  # 원문 링크가 있는 paragraph만 추출
        ...
        summary = paragraph.get_text()  # 요약문 추출
        ...
        original = re.sub(r'[\n\r\t]', '', original)
        original = re.sub(r"[^ㄱ-ㅎ가-힣ㅏ-ㅣa-zA-Z0-9.,? ]","",original) #- 한글,영어,숫자 및 .,? 를 제외한 특수문자 제외
        summary = re.sub(r"[^ㄱ-ㅎ가-힣ㅏ-ㅣa-zA-Z0-9.,? ]", "", summary)

        # csv의 row에 들어갈 data를 list변수로 생성

        data = [idx, category, title, date, summary, original]
        print(data)
