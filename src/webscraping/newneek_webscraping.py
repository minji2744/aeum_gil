...
def main():
    ...
    for idx, card in enumerate(cards):
        ...
        # card_window = browser.current_window_handle
        ...
        for paragraph in paragraphs:
            ...
            # 원문 기사
            try:
                original, url = original_text.get_original(url)
                original = re.sub(r'[\n\r\t]',' ', original)
        ...
        for window_handle in browser.window_handles:
            if window_handle != original_window:
                ...
                browser.close()
        browser.switch_to_window(original_window) # 첫번 째 탭(뉴닉 메인 페이지)으로 이동해서 다시 반복
